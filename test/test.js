...
tape( 'the function returns a single element containing a provided scalar value (dtype=float16)', function test( t ) {
	var expected;
	var actual;

	actual = array2scalar( 3.0, 'float16' );
	expected = new Float16Array( [ 3.0 ] );

	t.strictEqual( isSameFloat16Array( actual, expected ), true, 'returns expected value' );
	t.end();
});
